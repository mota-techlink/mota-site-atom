...
const SECTION_META: { icon: string; key: string }[] = [
  { icon: "🤖", key: "overview" },
  { icon: "🔁", key: "flow" },
  { icon: "📋", key: "prompt-fields" },
  { icon: "🗄️", key: "task-fields" },
  { icon: "⚡", key: "trigger" },
  { icon: "↩️", key: "back" },
];

function EngineDeepdiveDeckInner() {
  const c = useContent();
...
interface EngineDeepdiveDeckProps {
  access?: import("@/config/pitch-decks").DeckAccess;
  previewSlides?: number;
  isAuthenticated?: boolean;
  userRole?: string;
}

export function EngineDeepdiveDeck({
  access = "public",
  previewSlides = 3,
  isAuthenticated = false,
  userRole,
}: EngineDeepdiveDeckProps) {
  return (
    <DeckThemeProvider>
      <DeckAccessProvider
        access={access}
