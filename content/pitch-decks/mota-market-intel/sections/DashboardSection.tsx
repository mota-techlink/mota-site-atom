...
export function DashboardSection() {
  ...
  return (
    <section id="s-dashboard" className={`${SECTION} bg-zinc-950 relative`}>
      <DynamicBackground accent="cyan" brightness={1.4} count={16} />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
        {/* Header */}
        <div className="text-center mb-3 sm:mb-5">
          <span className="inline-block px-3 py-1 sm:px-4 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs sm:text-sm font-medium mb-2 sm:mb-3">
            {d.badge}
          </span>
        ...
        <div className="rounded-xl sm:rounded-2xl border border-white/10 bg-zinc-900/80 overflow-hidden">
          ...
          <div className="flex items-center gap-1.5 px-3 py-2 sm:px-4 sm:py-2.5 border-b border-white/5 bg-white/3">
            ...
            <span className="ml-2 text-[10px] sm:text-xs text-white/30">dashboard.motaiot.com</span>
          </div>
          ...
          <div className="p-3 sm:p-4 lg:p-5">
            ...
            <div className="mi-child grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-3 sm:mb-4">
              {d.stats.map((s: any, i: number) => (
                <div
                  key={i}
                  className="rounded-lg sm:rounded-xl bg-white/5 border border-white/8 p-2.5 sm:p-3.5"
                >
                  <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                    <span className="text-[10px] sm:text-xs text-white/40">{s.label}</span>
                    <span className="text-sm sm:text-base">{s.icon}</span>
                  </div>
                  <div className="text-lg sm:text-2xl font-bold text-white">{s.value}</div>
                  <div className="text-[10px] sm:text-xs text-emerald-400 mt-0.5">{s.change}</div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-2 sm:gap-3">
              ...
              <div className="mi-child lg:col-span-3 rounded-lg sm:rounded-xl bg-white/5 border border-white/8 p-2.5 sm:p-3.5">
                <div className="flex items-center justify-between mb-2 sm:mb-3">
                  <h4 className="text-[10px] sm:text-xs text-white/40 font-medium">
                    {d.chart.title}
                  </h4>
                  <span className="text-[9px] sm:text-[10px] text-cyan-400/60 font-medium flex items-center gap-1">
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1 w-full relative">
                          <div
                            className="absolute bottom-0 w-full rounded-sm bg-linear-to-t from-indigo-600 to-indigo-400 transition-all duration-700"
                            style={{
                        ...
                        <span className="text-[8px] sm:text-[10px] text-white/30 mt-1 shrink-0">
                          {bar.label}
                        </span>

                        {/* Hover tooltip */}
                        {isHovered && (
                          <div className="absolute -top-14 left-1/2 -translate-x-1/2 z-10 px-2.5 py-1.5 rounded-lg bg-zinc-800 border border-white/10 shadow-xl whitespace-nowrap pointer-events-none">
                            <div className="text-[10px] sm:text-xs font-bold text-white">
                              {bar.label}
                            </div>
                            <div className="text-[9px] sm:text-[10px] text-indigo-300">
                              🖱 {bar.value} clicks
                            </div>
                            <div className="text-[9px] sm:text-[10px] text-cyan-300">
                              📈 {bar.conv}%
                            </div>
                            <div className="absolute left-1/2 -translate-x-1/2 -bottom-1 w-2 h-2 rotate-45 bg-zinc-800 border-r border-b border-white/10" />
                          </div>
                        )}
                      </div>
                    );
              ...
              {/* Activity feed */}
              <div className="mi-child lg:col-span-2 rounded-lg sm:rounded-xl bg-white/5 border border-white/8 p-2.5 sm:p-3.5">
                <h4 className="text-[10px] sm:text-xs text-white/40 font-medium mb-2 sm:mb-3">
                  {d.activity.title}
                </h4>
                    ...
                    >
                      <div
                        className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full shrink-0 ${
                          STATUS_DOT[a.status] ?? "bg-white/20"
                        }`}
                      />
                      <span
                        className={`shrink-0 px-1.5 py-0.5 rounded text-[8px] sm:text-[10px] font-medium ${
                          PLATFORM_COLORS[a.platform]
                            ? `${PLATFORM_COLORS[a.platform]}/20 text-white/70`
                            : "bg-white/10 text-white/50"
                        }`}
                      >
                        {a.platform}
                      </span>
                      <span className="text-white/50 truncate flex-1">{a.action}</span>
