          ...
          {SECTION_IDS.map((_, idx) => {
            ...
            return (
              ...
              >
                ...
                {isLocked && (
                  <svg className="w-3.5 h-3.5 text-white/20 shrink-0" fill="currentColor" viewBox="0 0 20 20">
...
export function MobileTopBar({ onToggle }: { onToggle: () => void }) {
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-30 h-14 ${t.sidebarBg}/95 backdrop-blur-md border-b ${t.divider} flex items-center px-4 gap-3 lg:hidden`}>
      ...
      >
        <svg className={`w-6 h-6 ${t.subheading}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
