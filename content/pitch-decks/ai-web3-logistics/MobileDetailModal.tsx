...
export function MobileExpandButton({
  ...
  return (
    <motion.button
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-[10px] text-slate-400 font-mono backdrop-blur-sm hover:bg-white/10 transition-colors cursor-pointer ${className}`}
      onClick={onClick}
      whileTap={{ scale: 0.95 }}
    >
      <span>{label}</span>
