    ...
    <section id="s-reply" className={PAGE}>
      <div className={PAGE_INNER}>
        ...
        <div className="rounded-xl bg-white/5 border border-white/8 p-4 mb-8">
          <h3 className="text-base font-semibold text-white/50 mb-3">Prompt Rules</h3>
        ...
        {/* Channels */}
        <h3 className="text-base font-semibold text-white/60 mb-3">{d.channelsTitle}</h3>
          ...
          {d.channels.map((ch: any, i: number) => (
            <div key={i} className="rounded-xl bg-white/5 border border-white/8 p-4 hover:border-white/15 transition-colors">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-white/20 font-mono ml-auto">{ch.ref}</span>
              </div>
              <p className="text-xs text-white/40 leading-relaxed">{ch.desc}</p>
            </div>
          ))}
        </div>

        {/* Quality control */}
        <h3 className="text-base font-semibold text-white/60 mb-3">{d.qualityTitle}</h3>
