  ...
  return (
    <section id="s-partner" className={PAGE}>
      <div className={PAGE_INNER}>
        ...
        <h3 className="text-sm font-semibold text-white/60 mb-3">{d.programsTitle}</h3>
          ...
          {d.programs.map((p: any, i: number) => (
            <div key={i} className="rounded-xl bg-white/5 border border-white/8 p-4">
              ...
              <div className="space-y-1.5 text-[11px]">
                ...
                <div><span className="text-white/30">Tiers:</span> <span className="text-emerald-300/70 font-mono text-[10px]">{p.tiers}</span></div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* AI 7-stage */}
        <h3 className="text-sm font-semibold text-white/60 mb-3">{d.aiTitle}</h3>
        <div className="space-y-2 mb-8">
          {d.aiStages.map((s: any, i: number) => (
            <div key={i} className="flex items-start gap-3 group">
              <div className="w-7 h-7 shrink-0 rounded-full bg-gradient-to-br from-fuchsia-500/20 to-cyan-500/20 border border-fuchsia-500/25 flex items-center justify-center text-[10px] font-bold text-fuchsia-300">
                {s.stage}
              </div>
              <div className="flex-1 pt-0.5">
                <span className="text-xs font-bold text-white">{s.label}</span>
                <span className="text-[11px] text-white/35 ml-2">{s.desc}</span>
              </div>
        ...
        {/* Payments */}
        <h3 className="text-sm font-semibold text-white/60 mb-3">{d.paymentTitle}</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {d.payments.map((p: any, i: number) => (
            <div key={i} className="rounded-xl bg-white/5 border border-white/8 p-4">
              ...
              {p.chains && (
                <p className="text-[10px] text-cyan-300/50 font-mono">{p.chains}</p>
              )}
            </div>
          ))}
        </div>
