...
export function usePageNav(
  ...
  options?: { canView?: (idx: number) => boolean; onGated?: () => void },
) {
  ...
  const goTo = useCallback((idx: number) => {
    ...
    lockTimer.current = setTimeout(() => { locked.current = false; }, 1100);
    if (exitTimer.current) clearTimeout(exitTimer.current);
    exitTimer.current = setTimeout(() => { setExitingIdx(null); }, 1050);
  }, [options]);
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowDown" || e.key === "PageDown") { e.preventDefault(); if (!locked.current) goTo(activeIdxRef.current + 1); }
      if (e.key === "ArrowUp" || e.key === "PageUp") { e.preventDefault(); if (!locked.current) goTo(activeIdxRef.current - 1); }
      if (e.key === "Home") { e.preventDefault(); if (!locked.current) goTo(0); }
      if (e.key === "End") { e.preventDefault(); if (!locked.current) goTo(SECTION_IDS.length - 1); }
    };
  ...
  useEffect(() => {
    ...
    const onTouchEnd = (e: TouchEvent) => {
      ...
      if (Math.abs(dy) > SWIPE_THRESHOLD && dt < SWIPE_MAX_TIME && Math.abs(dy) > Math.abs(dx) * ANGLE_THRESHOLD_RATIO) {
        goTo(activeIdxRef.current + (dy < 0 ? 1 : -1));
      }
    };
