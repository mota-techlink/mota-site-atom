...
const COLOR_MAP: Record<string, { bg: string; border: string; badge: string; text: string }> = {
  ...
  amber:  { bg: "bg-cyan-950/40",  border: "border-cyan-400/30",  badge: "bg-cyan-500/20 text-cyan-200",  text: "text-cyan-200" },
  indigo: { bg: "bg-indigo-950/50", border: "border-indigo-400/30", badge: "bg-indigo-500/20 text-indigo-200", text: "text-indigo-200" },
};
...
export function ScheduleSection() {
  ...
  return (
    <section id="s-schedule" className={`${SECTION} bg-blue-950 relative`}>
      <DynamicBackground accent="amber" brightness={1.3} count={12} />
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-5 sm:mb-8">
          <span className="mi-child inline-block px-3 py-1 rounded-full bg-cyan-500/15 border border-cyan-400/30 text-cyan-300 text-xs sm:text-sm font-medium mb-2">{sc.badge}</span>
          <h2 className="mi-child text-white font-bold mb-2">{sc.title}</h2>
          <p className="mi-child text-blue-200/70 text-sm sm:text-base max-w-2xl mx-auto">{sc.subtitle}</p>
        </div>
          ...
          {sc.modes.map((mode: any, i: number) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-blue-200/60 text-[10px] sm:text-xs mb-1">{mode.time}</div>
      ...
      >
        {active ? (
          <>
            <div className="text-cyan-300 text-xs font-medium">{active.strategy}</div>
