    ...
    <section id="s-features" className={`${SECTION} bg-[#1a0f0a] relative`}>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-6 sm:mb-10">
          ...
          <p className="mi-child text-stone-300/70 text-sm sm:text-base max-w-2xl mx-auto">{f.subtitle}</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {f.items.map((item: any, i: number) => (
            <div
              ...
              className={`mi-child flex gap-4 rounded-xl bg-stone-900/60 border border-cyan-400/15 p-5 ${isMobile ? "cursor-pointer active:scale-[0.98] transition-transform" : ""}`}
            >
