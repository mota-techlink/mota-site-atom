...
export function ValueSection() {
  ...
  return (
    <section id="s-value" className={`${SECTION} bg-[#1a0f0a] relative`}>
      <DynamicBackground accent="indigo" brightness={1.3} count={14} />
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-6 sm:mb-10">
          <span className="mi-child inline-block px-3 py-1 rounded-full bg-amber-600/15 border border-amber-600/30 text-amber-300 text-xs sm:text-sm font-medium mb-2">{v.badge}</span>
          <h2 className="mi-child text-white font-bold mb-2">{v.title}</h2>
          <p className="mi-child text-stone-300/70 text-sm sm:text-base max-w-2xl mx-auto">{v.subtitle}</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {v.items.map((item: any, i: number) => (
            <div
              ...
              className={`mi-child rounded-xl bg-[#1a0f0a]/60 border border-indigo-400/25 p-5 sm:p-6 ${isMobile ? "cursor-pointer active:scale-[0.98] transition-transform" : ""}`}
            >
