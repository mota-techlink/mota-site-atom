...
const SECTION_META: { icon: string; key: string }[] = [
  { icon: "💰", key: "overview" },
  { icon: "📊", key: "tier-system" },
  { icon: "⚡", key: "x402" },
  { icon: "🏪", key: "platform-commission" },
  { icon: "🔧", key: "qa-billing" },
  { icon: "🧾", key: "settlement" },
  { icon: "🔍", key: "adjustment" },
  { icon: "↩️", key: "back" },
...
function BillingModelDeckInner() {
  const c = useContent();
...
interface BillingModelDeckProps {
  access?: DeckAccess;
...
export function BillingModelDeck({
  access = "public",
  previewSlides = DEFAULT_PREVIEW_SLIDES,
  isAuthenticated = false,
  userRole,
}: BillingModelDeckProps) {
  return (
    <DeckThemeProvider>
      <DeckAccessProvider
        access={access}
